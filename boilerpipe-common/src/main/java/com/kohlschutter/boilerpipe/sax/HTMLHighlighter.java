...
public class HTMLHighlighter {
	...
	public String process(final TextDocument doc, final InputSource is) throws BoilerpipeProcessingException {
		...
		/*
		 * if (outputHighlightOnly) { Matcher m;
		 * 
		 * boolean repeat = true; while (repeat) { repeat = false; m =
		 * PAT_TAG_NO_TEXT.matcher(html); if (m.find()) { repeat = true; html =
		 * m.replaceAll(""); }
		 * 
		 * m = PAT_SUPER_TAG.matcher(html); if (m.find()) { repeat = true; html
	...
	public String processTargetHtml(final TextDocument doc, final InputSource is,int channelId) throws BoilerpipeProcessingException {
		final Implementation implementation = new Implementation();
		implementation.process(doc, is);
		String html = implementation.targetHtml.toString();
		// actualContent=implementation.html.toString();
	...
	public String process(final BoilerpipeExtractor extractor, byte[] content, Charset cs)
			throws IOException, BoilerpipeProcessingException, SAXException {
	...
	public String process(final BoilerpipeExtractor extractor, byte[] content, Charset cs,int channelId)
			throws IOException, BoilerpipeProcessingException, SAXException {
		final HTMLDocument htmlDoc = new HTMLDocument(content, cs);
		final TextDocument doc = new BoilerpipeSAXInput(htmlDoc.toInputSource()).getTextDocument();
		extractor.process(doc,channelId);
		document = doc;
		final InputSource is = htmlDoc.toInputSource();
		return processTargetHtml(doc, is);
	}
	...
	public String processHtml() {
		/*
		 * final HTMLDocument htmlDoc = HTMLFetcher.fetch(url); final
		 * TextDocument doc = new
		 * BoilerpipeSAXInput(htmlDoc.toInputSource()).getTextDocument(); final
	...
	private String extraStyleSheet = "\n<style type=\"text/css\">\n" + ".x-boilerpipe-mark1 {"
			+ " text-decoration:none; " + "background-color: #ffff42 !important; " + "color: black !important; "
	...
	static {
		...
		TAG_ACTIONS.put("META", TA_IGNORABLE_ELEMENT);
	...
	private final class Implementation extends AbstractSAXParser implements ContentHandler {
		...
		StringBuilder appendableString = null;
		...
		public void startElement(String uri, String localName, String qName, Attributes atts) throws SAXException {
			...
			try {
				if (inIgnorableElement == 0) {
					...
					appendableString = new StringBuilder();
					appendableString.append('<');
					appendableString.append(qName);
					...
					if (!ignoreAttrs) {
						...
						for (int i = 0; i < numAtts; i++) {
							...
							appendableString.append(' '); appendableString.append(attr);
							appendableString.append("=\""); appendableString.append(data);
							appendableString.append("\"");
		...
		public void endElement(String uri, String localName, String qName) throws SAXException {
			...
			try {
				if (inIgnorableElement == 0) {
					...
					/*
					 * html.append("</"); html.append(qName); html.append('>');
					 */
				}
			} finally {
				if (ta != null) {
		      ...
		      if (inIgnorableElement == 0) {
		       ...
		       /* if (highlight) {
		        ...
		        if(highlight)
		        {
		            ...
		            targetHtml.append(appendableString.toString());
